...
type ResolverConfig struct {
	...
	SkipRouteFilter bool              `mapstructure:"skip_route_filter"`
...
// ResolverProvider returns the Polaris v3 resolver provider.
func ResolverProvider(load ResolverConfigLoader) yresolver.Provider {
	if load == nil {
		load = func(string) ResolverConfig { return ResolverConfig{} }
	}
	return yresolver.NewProvider("polaris", func(name string) (yresolver.Resolver, error) {
...
func (r *Resolver) fetchState(ctx context.Context, appName string) (yresolver.State, error) {
	...
	for _, inst := range resp.Instances {
		...
		if !metadataMatches(inst.GetMetadata(), r.cfg.Metadata) {
			continue
		}
		addr := common.NetAddr(inst.GetHost(), inst.GetPort())
...
func metadataMatches(instanceMetadata map[string]string, required map[string]string) bool {
	if len(required) == 0 {
		return true
	}
	for key, want := range required {
		if instanceMetadata[key] != want {
			return false
		}
	}
	return true
