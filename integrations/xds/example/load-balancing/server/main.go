...
func (s *LibraryImpl) ListShelves(
	...
	slog.Info("ListShelves called", "parent", req.PageSize, "server", s.serverID)
	_ = metadata.SetTrailer(ctx, metadata.Pairs("server", s.serverID))
	_ = metadata.SetHeader(ctx, metadata.Pairs("server", s.serverID))
	return &librarypb2.ListShelvesResponse{
		Shelves: []*librarypb2.Shelf{
			{Name: "shelves/1", Theme: "server-" + s.serverID + " 1"},
			{Name: "shelves/2", Theme: "server-" + s.serverID + " 2"},
		},
	}, nil
}
...
func (s *LibraryImpl) MergeShelves(
	...
	return &librarypb2.Shelf{
		Name:  req.Name,
		Theme: "Merged Theme - server-" + s.serverID,
	}, nil
}
...
func (s *LibraryImpl) GetBook(
	...
	slog.Info("GetBook called", "name", req.Name, "server", s.serverID)
	_ = metadata.SetTrailer(ctx, metadata.Pairs("server", s.serverID))
	_ = metadata.SetHeader(ctx, metadata.Pairs("server", s.serverID))
	return &librarypb2.Book{
		Name:   req.Name,
		Author: "Author - server-" + s.serverID,
		Title:  "Book Title - server-" + s.serverID,
		Read:   false,
	}, nil
}
...
func (s *LibraryImpl) ListBooks(
	...
	slog.Info("ListBooks called", "parent", req.Parent, "server", s.serverID)
	_ = metadata.SetTrailer(ctx, metadata.Pairs("server", s.serverID))
	_ = metadata.SetHeader(ctx, metadata.Pairs("server", s.serverID))
...
func (s *LibraryImpl) DeleteBook(
	...
	slog.Info("DeleteBook called", "name", req.Name, "server", s.serverID)
	_ = metadata.SetTrailer(ctx, metadata.Pairs("server", s.serverID))
	_ = metadata.SetHeader(ctx, metadata.Pairs("server", s.serverID))
	return &emptypb.Empty{}, nil
}
...
func (s *LibraryImpl) UpdateBook(
	...
	slog.Info("UpdateBook called", "book", req.Book.Name, "server", s.serverID)
	_ = metadata.SetTrailer(ctx, metadata.Pairs("server", s.serverID))
	_ = metadata.SetHeader(ctx, metadata.Pairs("server", s.serverID))
	return &librarypb2.Book{
		Name:   req.Book.Name,
...
func main() {
	...
	if err := yggdrasil.Init("github.com.codesjoy.yggdrasil.contrib.xds.example.load-balancing.server"); err != nil {
	...
	slog.Info("Starting load balancing server...", "server_id", serverID)

	ss := &LibraryImpl{serverID: serverID}
