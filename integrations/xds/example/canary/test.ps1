...
<#
.SYNOPSIS
    xDS Canary Deployment Test Script

.DESCRIPTION
    Automated test script for xDS canary deployment example.
#>
...
try {
    Write-ColorOutput "`n=== xDS Canary Deployment Test ===" "Cyan"

    Write-ColorOutput "`n=== Step 1: Starting xDS Control Plane ===" "Cyan"
    $env:XDS_CONFIG_FILE = "canary-xds-config.yaml"
    ...
    Write-ColorOutput "`n=== Step 2: Starting Stable Server ===" "Cyan"
    $env:DEPLOYMENT_TYPE = "stable"
    ...
    Start-Sleep -Seconds 2
    Write-ColorOutput "✓ Stable server started" "Green"

    Write-ColorOutput "`n=== Step 3: Starting Canary Server ===" "Cyan"
    ...
    Start-Sleep -Seconds 2
    Write-ColorOutput "✓ Canary server started" "Green"
    ...
    if ($clientOutput -match "Canary deployment test completed successfully") {
        Write-ColorOutput "✓ Test completed successfully" "Green"
        $successChecks++
    }

    if ($clientOutput -match "actual_canary_percentage") {
